function renderTimer(el){
  ...
  var h='<div class="sec-hdr"><div class="sec-title">&#9203; '+t('timer_title')+'</div>'
    +'<button class="btn danger sm" onclick="clearTimerLog()">'+t('clear_log')+'</button></div>';
  ...
  h+='<div class="timer-display">'
    ...
    +'<div class="timer-phase-label">'+(active?'&#9210; '+esc(tx(getPhase(active.phase).label)).toUpperCase():t('no_timer_running'))+'</div>'
    +'<div class="timer-btns">';
  if(active){
    h+='<button class="btn danger" onclick="stopTimer()">&#9632; '+t('stop_btn')+'</button>';
  }
  h+='</div></div>';
  ...
  PHASES.forEach(function(p){
    h+='<button class="tph-btn'+(APP.timerSelPhase===p.id?' on':'')+'" onclick="APP.timerSelPhase=\''+p.id+'\';renderTimer(document.getElementById(\'secbody\'))" style="'+(APP.timerSelPhase===p.id?'border-color:'+p.color+';color:'+p.color+';background:rgba(0,0,0,.3)':'')+'">'
      +p.icon+' '+esc(tx(p.short))+'</button>';
  });
  h+='</div>';

  if(!active){
    var selPhaseObj=getPhase(APP.timerSelPhase);
    var selLbl=selPhaseObj?tx(selPhaseObj.label):APP.timerSelPhase;
    h+='<div style="padding:10px 20px;border-bottom:1px solid var(--bdr);flex-shrink:0">'
      +'<button class="btn primary" onclick="startTimer()" style="width:100%">&#9654; '+t('start_timer_btn')+' — '+esc(selLbl.toUpperCase())+'</button>'
      +'</div>';
  }

  h+='<div class="timer-log">';
  if(!sessions.length){h+='<div class="empty">'+t('no_sessions_yet')+'</div>';}
  else{
    var grouped={};
    ...
    Object.keys(grouped).forEach(function(phase){
      ...
      var phLbl=ph.label?tx(ph.label):phase;
      h+='<div class="timer-row">'
        +'<span style="font-size:14px">'+ph.icon+'</span>'
        +'<span class="timer-row-ph">'+esc(phLbl)+'</span>'
    ...
    sessions.slice().reverse().slice(0,10).forEach(function(s){
      ...
      var phLbl=ph.label?tx(ph.label):s.phase;
      h+='<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-top:1px solid rgba(255,255,255,.03)">'
        ...
        +'<span style="font-size:11px;color:var(--t2);flex:1">'+esc(phLbl)+'</span>'
  ...
  h+='<div class="timer-total">'
    +'<span class="timer-total-lbl">'+t('total_engagement_time')+'</span>'
...
function clearTimerLog(){
  if(!confirm(t('clear_timer_confirm')))return;
  APP.timerSessions=[];
